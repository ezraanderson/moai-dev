...
int MOAICpBody::_applyForce ( lua_State* L ) {
	...
	cpBodyApplyForceAtLocalPoint( self->mBody, f, r );
...
int MOAICpBody::_applyImpulse ( lua_State* L ) {
	...
	cpBodyApplyImpulseAtLocalPoint( self->mBody, j, r );
...
int MOAICpBody::_getAngVel ( lua_State* L ) {
	...
	cpFloat av = cpBodyGetAngularVelocity ( self->mBody );
...
int MOAICpBody::_getPos ( lua_State* L ) {
	...
	cpVect p = cpBodyGetPosition ( self->mBody );
...
int MOAICpBody::_getRot ( lua_State* L ) {
	...
	cpVect r = cpBodyGetRotation ( self->mBody );
...
int MOAICpBody::_getVel ( lua_State* L ) {
	...
	cpVect v = cpBodyGetVelocity ( self->mBody );
...
int MOAICpBody::_isStatic ( lua_State* L ) {
	...
	cpBool isStatic = cpBodyGetType( self->mBody );
	lua_pushboolean ( state, ( isStatic == cpTrue ));
...
int MOAICpBody::_localToWorld ( lua_State* L ) {
	...
	v = cpBodyLocalToWorld ( self->mBody, v );
...
int MOAICpBody::_new ( lua_State* L ) {
	...
	//body->mBody->userData = body;
	cpBodySetUserData(body->mBody,body);
...
int MOAICpBody::_setAngVel ( lua_State* L ) {
	...
	cpBodySetAngularVelocity( self->mBody, av );
...
int MOAICpBody::_setPos ( lua_State* L ) {
	...
	cpBodySetPosition( self->mBody, p );
...
int MOAICpBody::_setVel ( lua_State* L ) {
	...
	cpBodySetVelocity( self->mBody, v );
...
int MOAICpBody::_worldToLocal ( lua_State* L ) {
	...
	v = cpBodyWorldToLocal ( self->mBody, v );
...
int MOAICpBody::_moveTo( lua_State* L ) {
MOAI_LUA_SETUP ( MOAICpBody, "UNN" )

  cpVect pos = cpBodyGetPosition ( self->mBody );
  float x1 = pos.x;
  float y1 = pos.y;
  ...
  // OFFSET ANGLE
	...
	cpBodySetVelocity( self->mBody, v );
	MOAICpBody* body = ( MOAICpBody* )self->mBody->userData;			
	body->ScheduleUpdate ();
...
void MOAICpBody::OnDepNodeUpdate () {
	...
	if ( this->mBody ) {
		...
		cpVect pos = cpBodyGetPosition ( this->mBody );
		cpVect rot = cpBodyGetRotation ( this->mBody );
