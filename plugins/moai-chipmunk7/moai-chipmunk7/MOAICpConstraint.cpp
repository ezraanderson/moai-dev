...
int MOAICpConstraint::_getMaxBias ( lua_State* L ) {
	...
	if ( self->mConstraint ) {
		...
		lua_pushnumber ( L, cpConstraintGetMaxBias(self->mConstraint ));
		return 1;
	}
	return 0;
}
...
int MOAICpConstraint::_getMaxForce ( lua_State* L ) {
	...
	if ( self->mConstraint ) {
		...
		lua_pushnumber ( L, cpConstraintGetMaxForce(self->mConstraint) );	
		return 1;
	}
	return 0;
}
...
int MOAICpConstraint::_newDampedRotarySpring ( lua_State* L ) {
	...
	cpConstraintSetUserData(constraint->mConstraint, &constraint);
...
int MOAICpConstraint::_setMaxBias ( lua_State* L ) {
	...
	if ( self->mConstraint ) {
		...
		cpFloat value_old = cpConstraintGetMaxBias(self->mConstraint );
		cpFloat value_new =  state.GetValue < cpFloat >( 2, value_old );
		cpConstraintSetMaxBias(self->mConstraint,value_new);
...
int MOAICpConstraint::_setMaxForce ( lua_State* L ) {
	...
	if ( self->mConstraint ) {
		...
		cpFloat value_old = cpConstraintGetMaxForce(self->mConstraint);
		cpFloat value_new =  state.GetValue < cpFloat >( 2, value_old );
		cpConstraintSetMaxForce(self->mConstraint,value_new);	
...
MOAICpConstraint::~MOAICpConstraint () {
	...
	if ( this->mConstraint ) {
		//MOAICpBody *con1 = (MOAICpBody *)cpBodyGetUserData(this->mConstraint);
		//MOAICpBody *con2 = (MOAICpBody *)cpBodyGetUserData(this->mConstraint);

	//MOAICpBody* body_a = cpConstraintGetBodyA(this->mConstraint);
//	MOAICpBody* body_b = cpConstraintGetBodyB(this->mConstraint);

		//this->LuaRelease (body_a );
		//this->LuaRelease (body_b );
		// this->mConstraint
