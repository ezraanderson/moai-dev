...
int MOAICpShape::_areaForPolygon ( lua_State* L ) {
	...
	//if ( numVerts && cpPolyValidate ( verts, numVerts )) {
		cpFloat area = cpAreaForPoly ( numVerts, verts,0.0f );
...
int MOAICpShape::_getBody ( lua_State* L ) {
	...
	if ( self->mShape && self->mShape->body ) {
		(( MOAICpBody* )self->mShape->body->userData )->PushLuaUserdata ( state );
		return 1;
	}
	return 0;
}
...
//***********************************************************************************
//FILTERS MADE OFF THE WORLD THEN ATTACHED TO SHAPE


//int MOAICpShape::_setFilter ( lua_State* L ) {
//
...
int MOAICpShape::_getSurfaceVel ( lua_State* L ) {
	...
	if ( self->mShape ) {
		...
		cpVect vel = cpShapeGetSurfaceVelocity(self->mShape);
...
int MOAICpShape::_getType ( lua_State* L ) {
	...
	//	lua_pushnumber ( L, self->mShape->collision_type );
		...
		cpCollisionType type = cpShapeGetCollisionType(self->mShape);
...
int MOAICpShape::_inside ( lua_State* L ) {
	...
	if ( self->mShape ) {
		...
		bool inside = cpShapePointQuery ( self->mShape, p,NULL ) == cpTrue ? true : false;
...
int MOAICpShape::_momentForPolygon ( lua_State* L ) {
	...
	//if ( numVerts && cpPolyValidate ( verts, numVerts )) {
		...
		lua_pushnumber ( L, cpMomentForPoly ( m, numVerts, verts, offset,0.0f ));
		return 1;
	//}
	//return 0;
}
...
int MOAICpShape::_momentForSegment ( lua_State* L ) {
	...
	cpFloat r = state.GetValue < cpFloat >( 6, 0 );

	lua_pushnumber ( L, cpMomentForSegment ( m, a, b ,r));
	return 1;
}
...
int MOAICpShape::_setElasticity ( lua_State* L ) {
	...
	if ( self->mShape ) {
		...
		//cpFloat value_old = cpShapeGetElasticity(self->mShape);
		//cpFloat value_new =  state.GetValue < cpFloat >( 2, value_old );
		//cpShapeSetElasticity(self->mShape,value_new);
	}
	return 0;
}
...
int MOAICpShape::_setFriction ( lua_State* L ) {
	...
	if ( self->mShape ) {
		...
		//cpFloat value_old = cpShapeGetFriction(self->mShape);
		//cpFloat value_new =  state.GetValue < cpFloat >( 2, value_old );
		//cpShapeSetFriction(self->mShape,value_new);
	}
	return 0;
}
...
int MOAICpShape::_setSurfaceVel ( lua_State* L ) {
	...
	if ( !self->mShape ) {
		...
		cpShapeSetSurfaceVelocity(self->mShape,surface_v);
...
int MOAICpShape::_setType ( lua_State* L ) {
	...
	if ( self->mShape ) {
		cpCollisionType value_old = cpShapeGetCollisionType(self->mShape);
		cpCollisionType value_new= state.GetValue < cpCollisionType >( 2, value_old );
		cpShapeSetCollisionType(self->mShape,value_new);
