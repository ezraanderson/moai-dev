...
/**	@name	setIndexBuffer
	...
	@in		MOAIEzraMesh self
	@in		MOAIIndexBuffer indexBuffer
	@out	nil
*/
int MOAIEzraMesh::_setIndexBuffer ( lua_State* L ) {
	MOAI_LUA_SETUP ( MOAIEzraMesh, "U" )
...
/**	@name	setPenWidth
	...
	@out	nil
*/
int MOAIEzraMesh::_setPenWidth ( lua_State* L ) {
	MOAI_LUA_SETUP ( MOAIEzraMesh, "UN" )
...
/**	@name	setPointSize
	...
	@out	nil
*/
int MOAIEzraMesh::_setPointSize ( lua_State* L ) {
	MOAI_LUA_SETUP ( MOAIEzraMesh, "UN" )
...
/**	@name	setPrimType
	...
	@in		MOAIEzraMesh self
	@in		number primType		One of MOAIEzraMesh GL_POINTS, GL_LINES, GL_TRIANGLES, GL_LINE_LOOP,
								GL_LINE_STRIP, GL_TRIANGLE_FAN, GL_TRIANGLE_STRIP
	@out	nil
*/
int MOAIEzraMesh::_setPrimType ( lua_State* L ) {
	MOAI_LUA_SETUP ( MOAIEzraMesh, "UN" )
...
/**	@name	setVertexBuffer
	...
	@out	nil
*/
int MOAIEzraMesh::_setVertexBuffer ( lua_State* L ) {
	MOAI_LUA_SETUP ( MOAIEzraMesh, "U" )

	//printf('');
	self->mVertexBuffer.Set ( *self, state.GetLuaObject < MOAIEzraVertexBuffer >( 2, true ));
	self->SetBoundsDirty ();
...
/**	@name	setVertexBuffer
	...
	@out	nil
*/
int MOAIEzraMesh::_setHigh ( lua_State* L ) {
	MOAI_LUA_SETUP ( MOAIEzraMesh, "U" )
...
//----------------------------------------------------------------//
ZLBox MOAIEzraMesh::ComputeMaxBounds () {
	return this->GetItemBounds ( 0 );
}

//----------------------------------------------------------------//
void MOAIEzraMesh::DrawIndex ( u32 idx, float xOff, float yOff, float zOff, float xScl, float yScl, float zScl ) {
	UNUSED ( idx );
	UNUSED ( xOff );
	UNUSED ( yOff );
	UNUSED ( zOff );
...
//----------------------------------------------------------------//
ZLBox MOAIEzraMesh::GetItemBounds ( u32 idx ) {
	UNUSED ( idx );
...
//----------------------------------------------------------------//
void MOAIEzraMesh::RegisterLuaClass ( MOAILuaState& state ) {
	...
	//state.SetField ( -1, "GL_POLYGON",			( u32 )ZGL_PRIM_POLYGON );
}

//----------------------------------------------------------------//
void MOAIEzraMesh::RegisterLuaFuncs ( MOAILuaState& state ) {
