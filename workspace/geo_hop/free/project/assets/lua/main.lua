...
print('>> LOAD DISPLAY', system['SCREEN_W'],system['SCREEN_H'],scale_width,scale_height,realW,realH)
...
--		mLayer['display']:insertProp ( draw_prop )
--
--
--print('********** END DRAW *******************')
--
...
for k,v  in pairs(tilemap['data']['layers']) do
	...
	if MOAIFileSystem.checkFileExists(system['pathImages']..'//tileset.png') then
		print('***************>>>>>>>>> FOUND IMAGE')
...
system['isFree'] 			= true --NEED TO BE A HOST CALL
...
then
		...
		if 	system['osType'] =='ios' then
				...
				MOAIChartBoost.init('547a633c04b01622ade3d6a0', '91d47edaa5c7fe99f03c105ed8e1a0407d266723')
		...
		elseif system['osType'] =='android' then
				...
				MOAIChartBoost.init('547a62e043150f10b0b8353b', '9a82c102567a08773cfe192ef59491af65c15951')
				MOAIChartBoost.loadInterstitial()
...
--*****************************************************
--BUG FOR FRAME BUFFER DELAY
	local delay_timer  = MOAITimer.new()
	delay_timer:setSpan(0.5)
	delay_timer:setMode(MOAITimer.NORMAL)
	delay_timer:setListener(MOAITimer.EVENT_TIMER_END_SPAN,function() print('DELAY BUG');load_game() end,0)
	delay_timer:start()
