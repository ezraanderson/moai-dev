  ...
  void main()
  {
    ...
    //} else {
      ...
      gl_FragColor = vec4((u_ambient+u_diffuse*diffuse) * texture2D(sampler, realTex).rgb + u_specular*specular*vec3(1.0,1.0,1.0), 1.0);
    	...
    	//gl_FragColor = texture2D ( sampler, realTex );
...
gfxQuadf = MOAIGfxQuad2D.new ()
gfxQuadf:setTexture ( "floor.png" )
gfxQuadf:setRect ( -1280*.5, -720*.5, 1280*.5, 720*.5 )
gfxQuadf:setUVRect ( 0, 0, 1, 1 )

propf = MOAIProp2D.new ()
propf:setDeck ( gfxQuadf )
layer:insertProp ( propf )
...
function threadFunc ()
      while true do
      local mx,my 		  =	 layer:wndToWorld (MOAIInputMgr.device.pointer:getLoc())           
      light:setLoc(mx, my, 10)
        coroutine.yield();
      end
end
